...
Он содержит методы:	
1. Метод handleResponse защищенный метод, который обрабатывает ответы от сервера. Если ответ успешен метод возвращает разобранный JSON-объект с 
...
### 1. Класс `IProduct`
Данный класс описывает модель продукта и наследует функциональность от класса Model, использует интерфейс IProduct для указания типа данных, 
